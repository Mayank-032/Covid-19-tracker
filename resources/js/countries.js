...
// Added on change button to show or hide the country list
chang_country_btn.addEventListener("click", function(){
    // Whenever we click on change button input text box will always be empty
    input.value = "";
    // Resets the country list
    // It is necessary to reset country list because whenever we -
    // -type in any country name in input box then we add or remove class hide
    resetCountryList();
    search_country_element.classList.toggle("hide");
    search_country_element.classList.toggle("fadeIn");
});

// When click on close button I need to hide the country list
...
// Country filter to search country by typing its name in text box
// Input event fires up only when value in it changes
// We will loop and check if the given value is found then we will -
// - remove hide class from it else we will add hide class in it
...
// Reset country list
/* It is necessary to reset country inorder to show every country
   in the country list. We have to do this because while typing
   country name in input field we add or remove hide class from it.*/
